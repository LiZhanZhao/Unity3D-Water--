...
//				Console.Write("\n");
		...
		private static string[] PrepareLightmaps(GameObject[] affectedObjects) {
			...
			foreach (int lightmapIndex in lightmapsToBackup) {
				Texture2D lightmapFar = LightmapSettings.lightmaps[lightmapIndex].lightmapLight;
			...
			for (int i = 0; i < lightmapPaths.Length; i++) {
				...
				if (!shouldUseOriginalLM) {
					...
					lmData.lightmapLight = AssetDatabase.LoadAssetAtPath( WPHelper.FilePathToAssetPath( lightmapPaths[i] ), typeof(Texture2D) ) as Texture2D;
					
					if (lmData.lightmapLight == null) {
						Debug.LogWarning("lmData.lightmapFar == null for " + WPHelper.FilePathToAssetPath( lightmapPaths[i] ));
						lmData.lightmapLight = LightmapSettings.lightmaps[i].lightmapLight;
					}
